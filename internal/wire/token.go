...
const (
	...
	DICTIONARY_3 byte = 239
	INTEROP_JID  byte = 245
	FB_JID       byte = 246
	AD_JID       byte = 247
	LIST_8       byte = 248
	LIST_16      byte = 249
...
// singleByteTokens is the SINGLE_BYTE_TOKENS table from constants.js.
// Index 0 is the empty string; valid token indices run 0..235.
...
// doubleByteTokens holds the four DOUBLE_BYTE_TOKENS dictionaries from constants.js.
// Each dictionary has 256 entries (indices 0..255).
// A double-byte token is encoded as (DICTIONARY_0+dict, idx).
...
// singleByteTokenMap is a reverse lookup: token string → index in singleByteTokens.
var singleByteTokenMap map[string]int

// doubleByteTokenMap is a reverse lookup: token string → (dict, idx).
type doubleTokenEntry struct{ dict, idx int }

var doubleByteTokenMap map[string]doubleTokenEntry

func init() {
	singleByteTokenMap = make(map[string]int, len(singleByteTokens))
	for i, s := range singleByteTokens {
		singleByteTokenMap[s] = i
	}
	total := 0
	for _, d := range doubleByteTokens {
		total += len(d)
	}
	doubleByteTokenMap = make(map[string]doubleTokenEntry, total)
	for d, dict := range doubleByteTokens {
		for i, s := range dict {
			doubleByteTokenMap[s] = doubleTokenEntry{d, i}
		}
	}
}

// tokenIndex returns the index of s in singleByteTokens, or ok=false if not found.
func tokenIndex(s string) (idx int, ok bool) {
	idx, ok = singleByteTokenMap[s]
	return
}

// doubleTokenIndex returns the dictionary number and index within that dictionary
// for s in doubleByteTokens, or ok=false if not found.
func doubleTokenIndex(s string) (dict int, idx int, ok bool) {
	e, ok := doubleByteTokenMap[s]
	if !ok {
		return 0, 0, false
	}
	return e.dict, e.idx, true
}
